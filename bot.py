...
if not TWELVE_DATA_API_KEY:
    log.error("Missing TWELVE_DATA_API_KEY env variable. Exiting.")
    sys.exit(1)
...
# ---------------------------------------------------------------------------
def fetch_price():
    """Fetch current price and 24h % change for XAU/USD from Twelve Data."""
    params = {"symbol": SYMBOL, "apikey": TWELVE_DATA_API_KEY}
    resp = requests.get(TWELVE_DATA_QUOTE_URL, params=params, timeout=10)
    ...
    if price is None:
        raise RuntimeError("No price field in response")
...
def poll_loop():
    ...
    while True:
        try:
            ...
            log.warning(
                "Price fetch failing for XAU/USD | Reason: %s | Consecutive failures: %d",
...
async def _send_failure_message():
    if time.time() < snooze_until:
        return
    text = (
        f"⚠️ Price fetch failing for XAU/USD\n"
...
async def cmd_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is None:
        ...
        return
    await update.message.reply_text(f"XAU/USD: {price:.1f}")
...
def main():
    ...
    log.info("Gold bot starting polling...")
