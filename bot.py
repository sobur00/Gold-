...
VS_CURRENCY = "usd"
OHLC_DAYS = 1  # 1 day range -> 30 min candles on CoinGecko free tier

POLL_SECONDS = 30
HISTORY_MAXLEN = 60  # keep last 60 closes for MA30 + buffer
...
# Shared state
# ---------------------------------------------------------------------------
closes = deque(maxlen=HISTORY_MAXLEN)
last_candle_ts = None
...
signal_history = deque(maxlen=360)  # ~3 hours at 30s polling, capped sensibly
snooze_until = 0
...
# ---------------------------------------------------------------------------
def fetch_ohlc():
    """Pull latest closed candle from CoinGecko OHLC endpoint."""
...
# ---------------------------------------------------------------------------
def poll_loop():
    global last_candle_ts, last_live_price, last_24h_change
    global consecutive_failures, last_fail_reason

    while True:
        try:
            ts, close_price = fetch_ohlc()
            price, change = fetch_live_price()

            with state_lock:
                # Only push a new close into history if it's a new candle
            ...
            log.info(
                "Poll OK | live=%.2f change=%.2f%% candle_close=%.2f",
                price, change if change is not None else 0.0, close_price,
            )
